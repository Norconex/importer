...
/**
 * <p>
 * Generic document parser factory. It uses Apacke Tika for <i>most</i> of
 ...
 *
 * <p>
 * In addition, since 2.6.0 you can control which embedded documents you
...
public class GenericDocumentParserFactory
    ...
    private static final Logger LOG = LoggerFactory.getLogger(GenericDocumentParserFactory.class);
    private static final String TIKA_TESSERACT_OCR_PARSER = "org.apache.tika.parser.ocr.TesseractOCRParser";
    private static final String TIKA_SQLITE_PARSER = "org.apache.tika.parser.jdbc.SQLite3Parser";

    private final Map<ContentType, IDocumentParser> parsers = new HashMap<>();
    ...
    private void loadParseHintsFromXML(XML xml) {
        ...
        // Grobid Config
        XML grobidXml = xml.getXML("grobid");
        if (grobidXml != null) {
            GrobidConfig grobidCfg = parseHints.getGrobidConfig();
            grobidCfg.setEnabled(grobidXml.getBoolean(
                    "@enabled", grobidCfg.isEnabled()));
    ...
    private void saveParseHintsToXML(XML xml) {
        ...
        GrobidConfig grobid = parseHints.getGrobidConfig();
        if (grobid.isEnabled()
                || !GrobidConfig.DEFAULT_SERVICE_URL.equals(
                        grobid.getServiceUrl())) {
