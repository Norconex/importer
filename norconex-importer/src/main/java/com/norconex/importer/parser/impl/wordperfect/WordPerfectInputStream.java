...
public class WordPerfectInputStream extends FilterInputStream {
	...
	public int read() throws IOException {
		...
		if (result >= 0x20 && result <= 0x7E) {
			...
			result = read();
		} else if (ArrayUtils.contains(INT_ARRAY, result)) {
...
//						8221 = ”
				...
				if (bytesRead == 2
						&& (result == 0xC3 && buf[1] == (byte) 0xC3 
						        || result == 0xC4 && buf[1] == (byte) 0xC4)
						&& (buf[0] == (byte) 0x08 || buf[0] == (byte) 0x0C 
						        || buf[0] == (byte) 0x0E)) {
				...
				// convert all other character to 0 to prevent StringExtractor
			    // to accept accented characters that were not encoded 
			    // and thus had another meaning
				result = 0;
			}
...
//			return 0x0D; // Line break
	 ...
	 * @throws IOException could not fill array
	 */
	public int fillByteArray(InputStream in, byte[] byteArray) 
	        throws IOException {
		int result = 0;
